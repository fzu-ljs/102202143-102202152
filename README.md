...
##### 3.成功运行会显示网址
##### 4.前端开发服务器会在  'http://localhost:8080'  启动。现在你可以通过谷歌浏览器访问  'http://localhost:8080/login'  来查看网站(第一次进入网站首先应该进入登录页面注册登录生成令牌，如果不是可能会导致功能无法使用，提示操作失败，而令牌存在有效期，当令牌过期后，那么你可以在谷歌浏览器打开 'http://localhost:8080/login' 重新登录)
...
## 四、登录与注册
1. 谷歌浏览器访问'http://localhost:8080/login' ，成功登陆网页后首先进入“登录”页面，若已有账号密码可直接登录，此时会跳出“登陆成功”弹窗，点击确定后进入“首页”。
...
2. 在该页面还可点击每个项目右下方的“修改”按钮修改项目标题、描述和要求（尚未真正实现），旁边的“删除”按钮可删除发布的项目(尚未真正实现)，点击“查看详情”按钮可查看项目详情。
...
## 八、我的项目页面
在“我的项目”页面，可以点击“退出项目”按钮(尚未真正实现)，也可点击项目查看项目描述和要求。
